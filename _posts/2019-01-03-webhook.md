...
You can configure webhooks for your MonitorQA account using either the web interface or the External API.

### Web Interface Setup

To configure webhooks via the web interface, follow these steps:
...
You can also configure webhooks programmatically using the External API. See the [Set webhook](#/set-webhook) endpoint documentation for details on configuring webhooks via the API.

Available endpoints:
- **Set or update webhook:** [Set webhook](#/set-webhook) (`POST /webhooks`)
- **Delete webhook:** [Delete webhook](#/delete-webhook) (`DELETE /webhooks`)
- **View webhook status:** [Get webhooks log](#/get-webhook-log) (`GET /webhooks/status`) to monitor webhook delivery
...
**Note:** The `data.correctiveAction` object contains [corrective action details](#/get-corrective-action), including `answer.note`, `answer.signature`, and `answer.files`, and is `null` when the event is `CORRECTIVE_ACTION_DELETED`.
